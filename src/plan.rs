...
pub fn make_plan(file_length: usize) -> impl Iterator<Item = (u64, usize)> {
    let full_chunks = file_length / CHUNK_SIZE;
    let full_chunks_used = full_chunks.checked_sub(1).unwrap_or_default();

    let tail_1_offset = full_chunks_used * CHUNK_SIZE;

    let remain_bytes = file_length - tail_1_offset;
    let remain_pages = remain_bytes / DEFAULT_PAGE_SIZE;

    let tail_1_len = if remain_bytes > CHUNK_SIZE {
        remain_pages.div_ceil(2) * DEFAULT_PAGE_SIZE
    } else {
        0
    };
    ...
    (0..full_chunks_used)
        .map(|x| (x * CHUNK_SIZE, CHUNK_SIZE))
        .chain(std::iter::once((tail_1_offset, tail_1_len)).filter(|(_, len)| *len > 0))
        .chain(std::iter::once((tail_2_offset, tail_2_len)))
...
mod test {
    ...
    fn test_make_plan() {
        ...
        assert_eq!(
            vec![(0, 17_245_233)],
            make_plan(17_245_233).collect::<Vec<_>>()
        );

        assert_eq!(vec![(0, 32 * M)], make_plan(32 * M).collect::<Vec<_>>());
